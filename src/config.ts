...
import { schemaI18n } from './locales/schema';
...
export const Config: Schema<Config> = Schema.object({
  nyaProfiler: Schema.object({
    apiKey: Schema.string()
      .role('secret')
      .required()
      .description('Re Nya Profiler 的 API Key（从 https://renya.mhtlim.top/ 中获取）')
  }).description('Re Nya Profiler API 配置'),

  pollTimeout: Schema.number().default(300).description('授权轮询超时时间（秒）'),
