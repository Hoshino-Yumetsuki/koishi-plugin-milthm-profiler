...
export const jaJP: Dict = {
  errors: {
    ...
    'auth-gen-parse-failed':
      '認証リンク生成レスポンスの解析に失敗しました。デバッグログを有効にしてください。',
    'auth-gen-failed': '認証リンクの生成に失敗しました: {detail}',
    'auth-poll-http-failed': '認証状態のポーリングに失敗しました: {detail}',
    'auth-poll-parse-failed':
      'ポーリングレスポンスの解析に失敗しました。デバッグログを有効にしてください。',
    ...
    'query-parse-failed':
      'クエリレスポンスの解析に失敗しました。デバッグログを有効にしてください。',
  ...
  commands: {
    milthm: {
      ...
      messages: {
        'no-binding':
          'Milthm アカウントが紐付けられていません。先に milthm.update で認証連携してください。',
        'no-cache':
          'ローカルキャッシュが見つかりません。先に milthm.update でデータを取得してください。',
    ...
    'milthm.update': {
      ...
      messages: {
        'auth-expired':
          '認証の有効期限が切れました。ブラウザで以下のリンクを開いて再認証してください（5分間有効）：',
        'auth-link-text': 'ここをクリックして認証',
        'query-failed': 'クエリ失敗: {message}',
        'pull-failed': '取得失敗: {message}',
        'bind-success-but-pull-failed':
          '連携成功（{username}）しましたが、データ取得に失敗しました: {message}',
        'auth-prompt':
          'ブラウザで以下のリンクを開いて {target} の認証連携を完了してください（5分間有効）：',
    ...
    'milthm.logout': {
      ...
      messages: {
        ...
        success:
          'ログアウトしました。連携データを削除しました。再使用するには milthm.update で再認証してください。',
