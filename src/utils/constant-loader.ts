...
type ConstantDataRawEntry =
  | [(number | string)?, (number | string)?, string?, string?, (number | string)?, ...unknown[]]
  | ConstantDataObjectEntry;

function isObjectEntry(entry: ConstantDataRawEntry): entry is ConstantDataObjectEntry {
...
export function loadConstantData(): Map<string, ConstantDataEntry> {
  ...
  for (const [chartId, rawEntry] of Object.entries(
    ...
    if (isObjectEntry(rawEntry)) {
      ...
      const [rawConstant, rawConstantV3, rawDifficulty, rawName, rawNoteCount] = rawEntry;

      const arr: [(number | string)?, (number | string)?, string?, string?, (number | string)?] = [
        rawConstant,
        rawConstantV3,
        rawDifficulty,
        rawName,
