...
function getCredentialsPath(baseDir: string, userId: string): string {
  return path.join(baseDir, 'data', PLUGIN_NAME, 'credentials', `${userId}.json`);
}
...
export function saveCredentials(baseDir: string, credentials: UserCredentials): void {
...
export function loadCredentials(baseDir: string, userId: string): UserCredentials | null {
...
export function loadRecord(baseDir: string, userId: string): UserSaveRecord | null {
